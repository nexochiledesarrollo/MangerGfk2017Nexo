...
var handleDataTableUsuarios = function() {
	...
	var table = $("#data-table").DataTable({
        ...
        "scrollX": true,
        ajax: {
            url: '/Manager/RestLoginUser/getListUSerByCliente/0',
...
var handleDetalleUsuario = function(id) {
    ...
    $('#txt_25').val(data.numero_user);
    $('#txt_26').val(data.dpto_user);
    $('#txt_27').val(data.casilla_user);
...
var createLoginUser = function(){
	...
	if(param.run == ''){
		...
		$('#etext-newlogin').html('Debes seleccionar lenguaje de Plataforma!');
		$("#error-newlogin").show();
		$("#txt_198").focus();
	}else{
